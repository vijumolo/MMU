  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-[#050505]">
      ...
      <header className="glass-panel sticky top-0 z-50 rounded-none border-t-0 border-l-0 border-r-0 shadow-md">
        ...
        <div className="container p-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-black/40 border border-cyan-500/30 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(34,211,238,0.3)]">
              <Trophy className="w-6 h-6 text-cyan-400" />
            </div>
            <div>
              <h1 className="text-xl m-0 text-gradient font-bold drop-shadow-md">UNISANGIL 2026</h1>
              <p className="text-xs text-secondary m-0 tracking-widest uppercase">Llegada por Categorías</p>
            </div>
          </div>
        </div>
      </header>
        ...
        <section className="glass-panel p-4 mb-8 animate-slide-up flex flex-col md:flex-row gap-4 items-center relative overflow-hidden">
          ...
          <div className="form-group flex-1 mb-0 w-full relative">
            ...
            <input 
              ...
              className="form-control pl-12 text-lg py-3 bg-black/40 border-cyan-500/30 focus:border-cyan-400 focus:shadow-[0_0_15px_rgba(34,211,238,0.2)]"
              value={searchTerm}
          ...
          <div className="form-group mb-0 w-full md:w-48 relative">
            <Users className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-secondary" />
            <select 
              className="form-control pl-12 py-3 bg-black/40 border-white/10"
              value={filterGender}
          ...
          {Object.keys(groupedData).length > 0 ? (
            <div className="space-y-10">
              {Object.keys(groupedData).sort().map((categoria) => (
                <div key={categoria} className="glass-panel overflow-hidden border border-white/10 shadow-2xl">
                  {/* Category Header */}
                  <div className="bg-gradient-to-r from-cyan-900/40 to-transparent p-4 border-b border-cyan-500/30 flex items-center justify-between">
                  ...
                  {Object.keys(groupedData[categoria]).sort().map((gender) => (
                    <div key={gender} className="mb-4">
                      ...
                      <div className="bg-black/60 px-6 py-3 border-b border-white/5">
                        <h3 className="text-lg font-semibold text-cyan-300 uppercase tracking-widest">{gender}</h3>
                      </div>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse whitespace-nowrap">
                          <thead>
                            <tr className="bg-white/5 border-b border-white/10 text-xs uppercase tracking-wider text-secondary">
                          ...
                          <tbody>
                            {groupedData[categoria][gender].sort((a,b) => a.posicion - b.posicion).map((r, i) => (
                              <tr 
                                key={r.dorsal + i} 
                                className="border-b border-white/5 hover:bg-cyan-900/20 transition-colors group"
                              >
                                <td className="p-4">{renderPositionBadge(r.posicion)}</td>
                                <td className="p-4 font-mono text-cyan-400 font-medium">{r.dorsal}</td>
                                <td className="p-4 text-white font-medium group-hover:text-cyan-100 transition-colors">{r.nombre}</td>
                                <td className="p-4 text-secondary">{r.club}</td>
                                <td className="p-4 text-yellow-400 font-mono font-bold">{r.tiempo}</td>
                                <td className="p-4 text-center">
                                  <button 
                                    onClick={() => handleDownloadDiploma(r.diplomaPage)}
                                    className="p-2 rounded bg-white/5 hover:bg-cyan-500/20 text-cyan-400 hover:text-cyan-300 border border-transparent hover:border-cyan-500/50 transition-all"
                                    title="Ver Diploma"
                                  >
            ...
            <div className="glass-panel p-10 text-center border-dashed border-white/20">
              <Medal className="w-12 h-12 text-secondary mx-auto mb-4 opacity-50" />
              <h3 className="text-xl text-white mb-2">No se encontraron resultados</h3>
              <p className="text-secondary">Intenta ajustando los filtros o escribiendo de otra forma.</p>
            </div>
          )}
        </section>
      </main>
      ...
      <footer className="glass-panel mt-auto rounded-none border-b-0 border-l-0 border-r-0 relative z-10 text-center p-6">
         <p className="text-secondary text-sm">© 2026 UNISANGIL. Todos los derechos reservados.</p>
         <p className="text-xs text-secondary/60 mt-1">Plataforma Oficial de Resultados | Diseño Avanzado</p>
      </footer>
    </div>
  );
}
